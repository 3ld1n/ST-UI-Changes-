...
function getCurrentGroupDisplayMembers() {
    ...
    const disabled = new Set(
        Array.isArray(group.disabled_members) ? group.disabled_members : []
    );

    const resolved = group.members
        .map((member, groupIndex) => {
            const avatar = typeof member === 'string' ? member : member?.avatar;
            if (!avatar) return null;

            const chid = ctx.characters.findIndex(character =>
                character?.avatar === avatar ||
                String(character?.name ?? '') === String(member?.name ?? '')
            );
            ...
            return {
                ...
                thumbnail: getThumbnail(ctx, character?.avatar || avatar),
                fullAvatar:
                    character?.avatar && character.avatar !== 'none'
                        ? `/characters/${encodeURIComponent(character.avatar)}`
                        : getThumbnail(ctx, character?.avatar || avatar),
                disabled: disabled.has(avatar),
            };
        })
        .filter(Boolean);

    // "Present" = enabled/unmuted members. If every member is disabled,
    // fall back to the full group instead of rendering an empty header.
    const present = resolved.filter(member => !member.disabled);
    return present.length ? present : resolved;
}

function buildGroupHeaderGrid(holder, members, title) {
    holder.innerHTML = '';

    if (!members.length) {
        const fallback = document.createElement('i');
        fallback.className = 'fa-solid fa-user-group';
        holder.appendChild(fallback);
        holder.setAttribute('aria-label', `${title || 'Group'} members`);
        return;
    }

    const grid = document.createElement('div');
    grid.className = 'em-group-avatar-grid';

    const count = members.length;
    if (count <= 4) {
        grid.classList.add(`em-group-count-${count}`);
    } else {
        grid.classList.add('em-group-count-many');
    }

    const visibleCount = count > 4 ? 3 : count;

    for (let index = 0; index < visibleCount; index++) {
        const member = members[index];
        const cell = document.createElement('div');
        cell.className = `em-group-avatar-cell em-group-avatar-cell-${index + 1}`;
    ...
    if (count > 4) {
        ...
        more.className = 'em-group-avatar-more';
    ...
    holder.setAttribute(
        'aria-label',
        `${title || 'Group'} — ${count} present ${count === 1 ? 'character' : 'characters'}`
    );
}

function closeGroupRosterDialog() {
    const dialog = document.querySelector('#em-group-roster-dialog');
    if (!dialog) return;
    ...
    try {
        ...
        // Removing it below is sufficient.
    }

    dialog.remove();
}

function showGroupRosterDialog() {
    const ctx = getContext();
    const group = findCurrentGroup(ctx);
    if (!group) {
    ...
    closeGroupRosterDialog();

    const members = getCurrentGroupDisplayMembers();
    if (!members.length) {
        globalThis.toastr?.info?.('No group characters are available.');
        return;
    }

    const dialog = document.createElement('dialog');
    dialog.id = 'em-group-roster-dialog';
    dialog.dataset.groupId = String(group.id ?? '');
    dialog.setAttribute('aria-labelledby', 'em-group-roster-title');

    const panel = document.createElement('div');
    panel.className = 'em-group-roster-panel';

    const header = document.createElement('div');
    header.className = 'em-group-roster-header';

    const heading = document.createElement('div');
    heading.className = 'em-group-roster-heading';

    const title = document.createElement('div');
    title.id = 'em-group-roster-title';
    title.className = 'em-group-roster-title';
    title.textContent = group.name || 'Group';

    const subtitle = document.createElement('div');
    subtitle.className = 'em-group-roster-subtitle';
    subtitle.textContent = `${members.length} present ${members.length === 1 ? 'character' : 'characters'}`;

    heading.append(title, subtitle);

    const close = document.createElement('button');
    close.type = 'button';
    close.className = 'em-group-roster-close';
    close.innerHTML = '&times;';
    close.setAttribute('aria-label', 'Close');
    close.addEventListener('click', closeGroupRosterDialog);

    header.append(heading, close);

    const grid = document.createElement('div');
    grid.className = 'em-group-roster-grid';
    if (members.length === 1) {
        grid.classList.add('em-group-roster-grid-single');
    }

    for (const member of members) {
        const card = document.createElement('div');
        card.className = 'em-group-roster-card';

        const imageWrap = document.createElement('div');
        imageWrap.className = 'em-group-roster-image-wrap';

        const img = document.createElement('img');
        img.className = 'em-group-roster-image';
        img.src = member.fullAvatar;
        img.alt = member.name;
        img.loading = 'eager';

        img.addEventListener('error', () => {
            if (img.dataset.emFallback === '1') return;
            img.dataset.emFallback = '1';
            img.src = member.thumbnail;
        });

        const name = document.createElement('div');
        name.className = 'em-group-roster-name';
        name.textContent = member.name;

        imageWrap.append(img, name);
...
function updateHeader() {
    ...
    const holder = header.querySelector('#em-header-avatar');
    const ctx = getContext();
    const group = findCurrentGroup(ctx);
    ...
    if (holder && group) {
        buildGroupHeaderGrid(holder, getCurrentGroupDisplayMembers(), title);
    } else {
        renderHeaderAvatar(avatar, title);
    }

    const roster = document.querySelector('#em-group-roster-dialog');
    if (roster && String(roster.dataset.groupId || '') !== String(group?.id || '')) {
        closeGroupRosterDialog();
    }
...
function bindHeaderInteractions() {
    ...
    if (headerAvatar && headerAvatar.dataset.emBound !== '1') {
        headerAvatar.dataset.emBound = '1';
        headerAvatar.setAttribute('role', 'button');
        headerAvatar.setAttribute('tabindex', '0');

        const openAvatarAction = () => {
            const group = findCurrentGroup(getContext());
            if (group) {
                showGroupRosterDialog();
            } else {
                openCurrentChatDetails();
            }
        };

        headerAvatar.addEventListener('click', openAvatarAction);
    ...
    if (headerText && headerText.dataset.emBound !== '1') {
        headerText.dataset.emBound = '1';
        headerText.setAttribute('role', 'button');
        headerText.setAttribute('tabindex', '0');

        headerText.addEventListener('click', openCurrentChatDetails);
...
function ensureScrollLatestSentinel() {
    ...
    let sentinel = chat.querySelector('#em-scroll-latest-sentinel');

    if (!sentinel) {
        sentinel = document.createElement('div');
        sentinel.id = 'em-scroll-latest-sentinel';
    ...
    */
    ...
    if (
        ...
        scrollLatestObserver = new IntersectionObserver(
            (entries) => {
                ...
                scrollLatestSentinelVisible = Boolean(entry?.isIntersecting);
                scheduleScrollLatestUpdate();
            },
            {
                root: chat,
    ...
    try {
        scrollLatestObserver.disconnect();
        scrollLatestObserver.observe(sentinel);
    } catch (error) {
        console.debug(`[${MODULE}] Scroll sentinel observer fallback`, error);
    }
...
function scheduleScrollLatestUpdate() {
    if (scrollLatestRaf) cancelAnimationFrame(scrollLatestRaf);
    scrollLatestRaf = requestAnimationFrame(updateScrollLatestButton);
}
...
function scrollChatToLatest() {
    ...
    */
    window.setTimeout(() => {
        try {
            (sentinel || lastMessage)?.scrollIntoView({
                behavior: 'smooth',
                block: 'end',
            });
        } catch {
...
function buildScrollLatestButton() {
    if (document.querySelector('#em-scroll-latest-button')) return;

    const button = makeButton(
        'em-scroll-latest-button',
...
function bindScrollLatestButton() {
    buildScrollLatestButton();

    const chat = document.querySelector('#chat');
    ensureScrollLatestSentinel();

    if (chat && chat.dataset.emScrollLatestBound !== '1') {
        chat.dataset.emScrollLatestBound = '1';
        chat.addEventListener('scroll', scheduleScrollLatestUpdate, { passive: true });
        chat.addEventListener('touchmove', scheduleScrollLatestUpdate, { passive: true });
    }
    ...
    */
    if (document.documentElement.dataset.emGlobalScrollLatestBound !== '1') {
        ...
        document.addEventListener('scroll', scheduleScrollLatestUpdate, {
            passive: true,
            capture: true,
        });

        document.addEventListener('touchmove', scheduleScrollLatestUpdate, {
            passive: true,
            capture: true,
        });
